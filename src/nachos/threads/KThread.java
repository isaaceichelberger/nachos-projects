...
public class KThread {
	...
	// If a thread was joined, we need to wake up the other thread
		boolean intStatus = Machine.interrupt().disable();
		// get the thread from the threadqueue and make it ready
		KThread threadToWake = threadQueue.nextThread();
		if (threadToWake != null){
			threadToWake.ready();
		}
		Machine.interrupt().restore(intStatus);

		sleep(); // current thread's status is finished, will run the next thread
    ...
    public void join() {
		...
		if (this.status == statusFinished){
			return; // immediately return from join if called on an already finished thread
		}
    ...
    // thread with access should receive priority
}
